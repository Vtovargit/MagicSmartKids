...
public class UserController {
    ...
    /**
     * DEBUG: Endpoint temporal para verificar autenticación
     */
    @GetMapping("/debug")
    ...
    /**
     * EMERGENCY: Endpoint sin restricciones para debugging crítico
     */
    @GetMapping("/emergency")
    public ResponseEntity<?> emergencyUsers() {
        try {
            System.out.println("🚨 EMERGENCY: Acceso de emergencia a usuarios");
            
            List<User> users = userRepository.findAll();
            
            Map<String, Object> response = new HashMap<>();
            response.put("success", true);
            response.put("message", "Acceso de emergencia - usuarios obtenidos");
            response.put("users", users);
            response.put("total", users.size());
            response.put("warning", "Este es un endpoint de emergencia sin autenticación");
            
            return ResponseEntity.ok(response);
            
        } catch (Exception e) {
            System.err.println("❌ Error en endpoint de emergencia: " + e.getMessage());
            
            Map<String, Object> response = new HashMap<>();
            response.put("success", false);
            response.put("message", "Error en acceso de emergencia: " + e.getMessage());
            return ResponseEntity.badRequest().body(response);
        }
    }

    /**
     * TEMPORAL: Listar usuarios con menos restricciones para debugging
     */
    @GetMapping("/temp")
    @PreAuthorize("hasAnyRole('COORDINATOR','ADMIN','TEACHER')")
    public ResponseEntity<?> getAllUsersTemp(Authentication authentication) {
        try {
            System.out.println("🔧 TEMP: Listando usuarios con permisos ampliados");
            ...
            Map<String, Object> response = new HashMap<>();
            response.put("success", true);
            response.put("message", "Usuarios obtenidos correctamente (temporal)");
            response.put("users", users);
            response.put("total", users.size());
            ...
            System.err.println("❌ Error en endpoint temporal: " + e.getMessage());
    ...
    /**
     * Listar todos los usuarios - Ampliado temporalmente para más roles
     */
    @GetMapping
    @PreAuthorize("hasAnyRole('COORDINATOR','ADMIN','TEACHER','SECRETARY')")
    ...
    /**
     * Obtener usuarios por rol - Ampliado temporalmente para más roles
     */
     ...
     */
    @GetMapping("/{id}")
    @PreAuthorize("hasAnyRole('COORDINATOR','ADMIN') or @userRepository.findById(#id).orElse(null)?.email == authentication.name")
