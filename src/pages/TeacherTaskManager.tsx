...
const TeacherTaskManager: React.FC = () => {
    ...
    try {
      const response = await fetch('/api/tasks', {
        ...
        body: JSON.stringify({
          ...
          teacherId: user?.id,
          institutionId: (user as any)?.institution?.id,
  ...
  const loadTasks = async () => {
    try {
      const institutionId = (user as any)?.institution?.id;
      if (!institutionId) {
        setTasks([]);
        return;
      }
      ...
      alert('Error de conexión al cargar tareas: ' + (error?.message || 'Desconocido'));
    }
  };
