...
import { 
  ...
  CheckCircle2, AlertTriangle, Settings, Plus, FileText, 
  ...
  const loadStats = async () => {
    try {
      ...
      if (res.ok) {
        ...
        console.error('Error loading stats:', res.status, res.statusText);
      }
    } catch (error) {
  ...
  const loadInstitutions = async () => {
    try {
      ...
      if (res.ok) {
        ...
        console.error('Error loading institutions:', res.status, res.statusText);
      }
    } catch (error) {
  ...
  const loadRecentActivity = async () => {
    try {
      ...
      if (res.ok) {
        ...
        console.error('Error loading activities:', res.status, res.statusText);
      }
    } catch (error) {
  ...
  const handleQuickAction = (action: string) => {
    switch (action) {
      ...
      case 'settings':
        ...
        break;
      case 'monitoring':
        alert('Función de monitoreo en desarrollo');
        break;
      case 'backup':
