...
const AdminDashboard: React.FC = () => {
  ...
  const toggleInstitutionActive = async (id: string, makeActive: boolean) => {
    try {
      const updated = await toggleInstitutionActiveRequest(id, makeActive, token);
      setInstitutionStats(prev => prev.map(inst => inst.id === String(updated.id)
        ? { ...inst, status: updated.isActive === false ? 'inactive' : 'active' }
        : inst
      ));
    } catch (e) {
      console.error('Error updating institution status', e);
    }
  };
  ...
  const loadStats = async () => {
    try {
      ...
      const headers = { 'Authorization': `Bearer ${localStorage.getItem('token')}` } as HeadersInit;

      const [instRes, studentsRes, teachersRes, healthRes] = await Promise.all([
      ...
      const institutionsJson = instRes.ok ? await instRes.json() : { institutions: [] };
      const studentsJson = studentsRes.ok ? await studentsRes.json() : { users: [] };
      const teachersJson = teachersRes.ok ? await teachersRes.json() : { users: [] };
      const healthJson = healthRes.ok ? await healthRes.json() : null;

      const totalInstitutions = (institutionsJson.institutions || []).length;
      const activeStudents = (studentsJson.users || []).length;
      const activeTeachers = (teachersJson.users || []).length;
      ...
      setStats({
        ...
        activeTeachers,
        monthlyGrowth: '+0%',
        systemUptime: healthJson?.status === 'UP' ? 'UP' : 'DOWN',
        pendingApprovals: 0,
        systemHealth: healthJson?.status === 'UP' ? 'good' : 'critical',
        loading: false
      });
    } catch (error) {
  ...
  const loadRecentActivity = async () => {
    try {
      ...
      // No hay endpoint real aún; mostrar vacío sin mocks
      setRecentActivity([]);
    } catch (error) {
  ...
  const loadInstitutionStats = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const list = (data.institutions || []).map((inst: any, idx: number) => ({
    ...
    <div className="space-y-4 sm:space-y-6 lg:space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3">
          ...
          <Button onClick={() => setIsCreateOpen(true)} className="bg-primary hover:bg-primary-600 text-neutral-white border-0 flex items-center gap-2 justify-center">
                    ...
                    <td className="py-3 px-2 sm:px-4">
                      <div className="flex gap-2">
                        <Button 
                          ...
                          onClick={() => toggleInstitutionActive(institution.id, institution.status !== 'active')}
                        >
      ...
      <CreateInstitutionModal 
        ...
        onInstitutionCreated={(inst) => {
          // Refrescar lista con la institución creada
          setInstitutionStats(prev => [
            { id: String(inst.id), name: inst.name, students: 0, teachers: 0, avgGrade: 0, status: inst.isActive === false ? 'inactive' : 'active' },
            ...prev
          ]);
        }}
      />
...
// Helpers
async function toggleInstitutionActiveRequest(id: string, makeActive: boolean, token: string | null) {
